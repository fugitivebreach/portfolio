...
// Display Lavalink track in player
function displayLavalinkTrack(track) {
    const musicPlayerContent = document.getElementById('musicPlayerContent');
...
function simulatePlayback() {
    if (!lavalinkPlayer || !lavalinkPlayer.isPlaying) return;
    ...
    const interval = setInterval(() => {
        ...
        if (currentTimeDisplay && currentTrack) {
            const currentMs = (progress / 100) * currentTrack.duration;
            currentTimeDisplay.textContent = formatDuration(currentMs);
        }
    }, 500);
}
